CREATE VIEW [dbo].[v_registration_latest_accepted_ST_1]
AS select *
from
(
	SELECT  distinct
...
ELSE NULL END as organisation_type_code_description,
	...
	cs.created,
	--updated to order on the submission_event created date i.e. the decision date rather than the cs created which is the submitted date 
	...
	--Join added to restrict the data set down to just those files which have a status of accepted
	INNER JOIN [rpd].[SubmissionEvents] se on se.fileid = cs.fileid and se.[type] = 'RegulatorRegistrationDecision' AND se.[Decision] = 'Accepted'
)A
where A.rn = 1;