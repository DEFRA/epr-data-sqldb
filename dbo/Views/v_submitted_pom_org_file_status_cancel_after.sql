CREATE VIEW [dbo].[v_submitted_pom_org_file_status_cancel_after] AS With
/****************************************************************************************************************************
	History: 
...
SubmissionEvents_interim as
(
	select se.*, fs.fileid_new, ri.IsResubmission_identifier
	...
	left join resubmission_ids ri on ri.FileId = ISNULL(se.FileId,fs.fileid_new) 
),
cancelled_resubmission_fileids as 
(
select SubmissionId, Created, Decision, ISNULL(fileid, fileid_new) as fileid_new 
from SubmissionEvents_interim
where IsResubmission_identifier = 1 and Type = 'RegulatorRegistrationDecision' and Decision = 'Cancelled'
),
SubmissionEvents_updated as 
(
	select S.*,
				case when S.IsResubmission_identifier = 0 and S.Decision = 'Accepted' and C.SubmissionId is not null then ISNULL(C.Decision,S.Decision) 
					 when S.IsResubmission_identifier = 1 and S.Decision = 'Cancelled' and C.SubmissionId is not null then NULL
					else S.Decision
					end as Decision_recalculated
					,
				case when S.IsResubmission_identifier = 0 and S.Decision = 'Accepted' and C.SubmissionId is not null then ISNULL(C.Created,S.Created) 
					 when S.IsResubmission_identifier = 1 and S.Decision = 'Cancelled' and C.SubmissionId is not null then NULL
					else S.Created
					end as Created_recalculated
					,
				case when S.IsResubmission_identifier = 0 and S.Decision = 'Accepted' and C.SubmissionId is not null then ISNULL(C.fileid_new,S.fileid_new) 
					 when S.IsResubmission_identifier = 1 and S.Decision = 'Cancelled' and C.SubmissionId is not null then NULL
					else ISNULL(S.fileid, S.fileid_new)
					end as fileid_new_recalculated
	from SubmissionEvents_interim S
	left join cancelled_resubmission_fileids C on C.SubmissionId = S.SubmissionId 
	where S.Type = 'RegulatorRegistrationDecision' 
),
...
(
	select distinct sub.submitted_Fileid as app_submitted_Fileid, sub.SubmissionEventId_of_submitted_record
		...
		, app_sub.Created as Submitted_ts
...
,res as (
		select distinct
				...
				, se.created as Decision_Date_Original
				, se.Created_recalculated as Decision_Date
				, case when Right(dbo.udf_DQ_SubmissionPeriod(s.SubmissionPeriod),4) >= 2025 and ISNULL(rid.IsResubmission_identifier,0) = 0 and se.Decision = 'Accepted' Then 'Granted'
					...
					as Regulator_Status_Old
				, case when Right(dbo.udf_DQ_SubmissionPeriod(s.SubmissionPeriod),4) >= 2025 and ISNULL(rid.IsResubmission_identifier,0) = 0 and se.Decision_recalculated = 'Accepted' Then 'Granted'
					   when Right(dbo.udf_DQ_SubmissionPeriod(s.SubmissionPeriod),4) >= 2025 and ISNULL(rid.IsResubmission_identifier,0) = 0 and se.Decision_recalculated = 'Rejected' Then 'Refused'
					   when app_submitted.SubmissionEventId_of_application_submitted_record is not null and se.Decision_recalculated is null then 'Pending'
					   when app_submitted.SubmissionEventId_of_application_submitted_record is null and se.Decision_recalculated is null and Right(dbo.udf_DQ_SubmissionPeriod(s.SubmissionPeriod),4) >= 2025 then 'Uploaded'
					   when app_submitted.SubmissionEventId_of_application_submitted_record is null and se.Decision_recalculated is null and Right(dbo.udf_DQ_SubmissionPeriod(s.SubmissionPeriod),4) < 2025  then 'Pending'
					   else se.Decision_recalculated
					   end
					as Regulator_Status
				...
				, se.UserId
				, cfm.Created
				, app_submitted.Submitted_ts as 'Application_submitted_ts'
				...
				, se.Decision_recalculated as Original_Regulator_Status_recalculated_for_cancel_scenario
				,s.SubmissionType
...
select distinct pom_cd.SubmissionId, pom_cd.RegistrationSetId, pom_cd.OrganisationId, pom_cd.FileName, pom_cd.FileType, pom_cd.OriginalFileName, pom_cd.TargetDirectoryName, pom_cd.Decision_Date_Original, pom_cd.Decision_Date, pom_cd.Regulator_Status_Old, pom_cd.Regulator_Status, pom_cd.RegulatorDecision, pom_cd.Regulator_User_Name, pom_cd.Regulator_Rejection_Comments, pom_cd.RejectionComments, pom_cd.Type, pom_cd.UserId, pom_cd.RowNumber, pom_cd.Created, pom_cd.Application_submitted_ts, pom_cd.RegistrationType, pom_cd.SubmissionPeriod, pom_cd.ApplicationReferenceNo, pom_cd.registrationreferencenumber, pom_cd.Original_Regulator_Status_recalculated_for_cancel_scenario, pom_cd.Original_Regulator_Status, pom_cd.SubmissionType, pom_cd.IsResubmission_identifier, pom_cd.cfm_FileId, pom_cd.FileId, pom_cd.fileid_new, pom_cd.submitted_Fileid, pom_cd.SubmissionEventId_of_submitted_record, pom_cd.app_submitted_Fileid, pom_cd.SubmissionEventId_of_application_submitted_record
from cd_pom_result pom_cd
union
select distinct cfm.SubmissionId, cfm.RegistrationSetId, cfm.OrganisationId, cfm.FileName, cfm.FileType, cfm.OriginalFileName, cfm.TargetDirectoryName, cp.Decision_Date_Original, cp.Decision_Date, cp.Regulator_Status_Old, cp.Regulator_Status, cp.RegulatorDecision, cp.Regulator_User_Name, cp.Regulator_Rejection_Comments, cp.RejectionComments, cp.Type, cp.UserId, cp.RowNumber, cp.Created, cp.Application_submitted_ts, cp.RegistrationType, cp.SubmissionPeriod, cp.ApplicationReferenceNo, cp.registrationreferencenumber, cp.Original_Regulator_Status_recalculated_for_cancel_scenario, cp.Original_Regulator_Status, cp.SubmissionType, cp.IsResubmission_identifier, cp.cfm_FileId, cp.FileId, cp.fileid_new, cp.submitted_Fileid, cp.SubmissionEventId_of_submitted_record, cp.app_submitted_Fileid, cp.SubmissionEventId_of_application_submitted_record
From rpd.cosmos_file_metadata cfm
