/****** Object:  View [dbo].[v_latest_accepted_orgfile_by_year]    
Script Date: 15/05/2026 09:56:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[v_latest_accepted_orgfile_by_year] 
AS WITH 
base_data
AS (
...
CASE 
	 WHEN m.SubmissionPeriod LIKE 'January to December %'
     THEN CAST(RIGHT(m.SubmissionPeriod,4) AS INT) - 1
     ELSE CAST(RIGHT(TRIM(m.SubmissionPeriod),2) AS INT) + 2000
END AS ReportingYear,
