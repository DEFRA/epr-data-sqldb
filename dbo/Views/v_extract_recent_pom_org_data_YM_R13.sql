CREATE VIEW [dbo].[v_extract_recent_pom_org_data_YM_R13] AS with
/****************************************************************************************************************************
	History:
...
f_org_sql as
 ...
 l_org_sql as
 (select [Org ID],	Rank,	ReportingYear	,[Submission date time],	[Submitted by]	,[Submission status],	[Regulator Decision],	[Actual Regulator Decision],	[Who submitted],[CS Nation],	cd_filename,	ComplianceSchemeId,	cd_organisation_size--,	cd_submission_period_code ,IsResubmission_identifier 
	from (select a.*, row_number() over(partition by [Org ID], ReportingYear,[Rank] order by [Submission date time] desc) as Lastest_status from 
 (
...
submission_count as
(
	select  [Org ID] ,ReportingYear, count([Org ID]) as orgcount
	from
	(
