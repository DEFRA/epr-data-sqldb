CREATE VIEW [dbo].[v_CrossValidation_Landing_Page] AS
...
all_CompanyDetails as (
    select *
        ,Row_number() over(partition by OrganisationId, RelevantYear, RegistrationJourney order by Submission_time desc) as cd_rn
    from base_data
    where upper(FileType) = 'COMPANYDETAILS'
        and (
            ComplianceSchemeId is not null
            or (
                ComplianceSchemeId is null
                and UPPER(TRIM(ISNULL(Regulator_Status,'PENDING'))) in ('PENDING', 'ACCEPTED', 'GRANTED', 'queried'))
            )
...
select opc.*, np.Name AS ProducerNationName
