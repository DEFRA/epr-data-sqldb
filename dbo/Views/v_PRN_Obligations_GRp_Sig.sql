CREATE VIEW [dbo].[v_PRN_Obligations_GRp_Sig]
AS With 
org As (  
/****************************************************************************************************************************
	History:
...
obgns As (
	...
	Left Join
		prnLtst		pl	on ob.SubmitterId=pl.OrganisationId 
	Left Join
		pmm 
...
/*** ^^^^ ** SN002 ** ^^^^ ***/
	Select 
		...
		,RemainStatus = Case When Sum(o.MaterialObligationValue) - Sum(IsNull(p.AcceptedTonnage,0)) >0 Then 'Not Met' Else 'Not'	End	
	From
		obgnsgrp		o
	Left Join
		prntt			p		
