CREATE VIEW [dbo].[v_extract_recent_pom_org_data_ts] AS with
/****************************************************************************************************************************
	History:
...
(
		...
		from 
		(
			select distinct o.id as OrganisationId, cast(cd.organisation_id as int) as ReferenceNumber
			...
			left join [rpd].[Organisations_pom_extract] o on o.ReferenceNumber = cd.organisation_id
			left join [rpd].[cosmos_file_metadata_pom_extract] cfm on cfm.FileName = cd.FileName
...
(
		...
		from 
		(
			select distinct o.id as OrganisationId, cast(pm.organisation_id as int) as ReferenceNumber
					, case when cfm.SubmissionPeriod in ('Jan to Jun 2023','January to June 2023') then 1 
							when cfm.SubmissionPeriod = 'July to December 2023' then 2
							when cfm.SubmissionPeriod in ('Jan to Jun 2024','January to June 2024') then 3 
					...
					, fs.IsResubmission_identifier --YM006
			from [rpd].[Pom] pm
			left join [rpd].[Organisations_pom_extract] o on o.ReferenceNumber = pm.organisation_id
			left join [rpd].[cosmos_file_metadata_pom_extract] cfm on cfm.FileName = pm.FileName
			left join [rpd].[ComplianceSchemes] cs on cs.ExternalId = cfm.ComplianceSchemeId
			left join rpd.users u on u.USerId = cfm.UserId
			left join rpd.persons p on p.UserId = u.id
			left join rpd.Nations N on N.Id = cs.NationId
...
(
	...
	(
		...
		, CONVERT(DATETIME,substring(cfm.Created,1,23)) as Submission_time
		, cfm.FileName
		From [rpd].[cosmos_file_metadata_pom_extract] cfm
...
*/
 ...
 (
	...
	from [rpd].[Organisations_pom_extract] O
...
-- ,submission_count as
--(
--	select distinct [Org ID],ReportingYear, 0 as cnt
--	from
--	(
--		select cast([Org ID] as nvarchar(6)) as [Org ID],ReportingYear from l_org_sql
--		union all 
--		select cast([Org ID] as nvarchar(6)) as [Org ID],ReportingYear From l_pom_sql
--	) A 
...
(
	...
	from [rpd].[Organisations_pom_extract] Org
	...
	--left join submission_count sub_c on sub_c.[Org ID] = Org.ReferenceNumber and sub_c.ReportingYear = TwoRow.Reporting_Year
...
select 
	...
	--, case when sub_c.cnt = 4 then 'Y' else 'N' end as Reported_mandated_data_sets
	--,sub_c.cnt as Reported_mandated_data_sets
	--,bs.Reported_mandated_data_sets
