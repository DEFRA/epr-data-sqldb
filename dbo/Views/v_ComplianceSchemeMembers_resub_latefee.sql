CREATE VIEW [dbo].[v_ComplianceSchemeMembers_resub_latefee]
...
(
select * from (select base.*,
		   ROW_NUMBER() OVER (
			PARTITION BY OrganisationId, ComplianceSchemeId, SubmissionPeriod
			ORDER BY EarliestSubmissionDate ASC
		   ) AS RowNum
	from (
		select OrganisationId, ComplianceSchemeId, SubmissionPeriod,FileId as EarliestFileId, min (SubmittedDate_datetime) as EarliestSubmissionDate
		from base_data
		group by OrganisationId, ComplianceSchemeId, SubmissionPeriod, FileId
	) as base
	) col_base where RowNum = 1
)
