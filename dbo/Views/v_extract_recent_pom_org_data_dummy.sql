CREATE VIEW [dbo].[v_extract_recent_pom_org_data_dummy] AS with 
/****************************************************************************************************************************
	History:
...
--select * from ORG_LATEST_IS_NOT_QUERIED
ORG_PENDING_ACCEPT_ONLY_updated as
(
select Y.*
, row_number() over(partition by y.OrganisationId, y.ReferenceNumber, y.SubmissionPeriod order by y.Submission_time asc, Source asc) as First_pending_accepted_submission_updated
		, row_number() over(partition by y.OrganisationId, y.ReferenceNumber, y.SubmissionPeriod order by y.Submission_time desc, Source asc) as Last_pending_accepted_submission_updated 
		from ORG_PENDING_ACCEPT_ONLY Y
left join ORG_LATEST_IS_NOT_QUERIED Q on Y.OrganisationId = Q.OrganisationId and Y.ReferenceNumber = Q.ReferenceNumber and Y.SubmissionPeriod = Q.SubmissionPeriod
where Q.OrganisationId is null 
		or 
		(Y.Actual_Regulator_Status <> 'QUERIED' and  Q.OrganisationId is not null)
),

--select * from ORG_PENDING_ACCEPT_ONLY_updated where referencenumber = '126500'
...
--select * from f_org_sql where [Org ID] = '126500'
 ,
l_org_sql as
 (
	select ReferenceNumber as 'Org ID', SubmissionPeriod as 'Rank', ReportingYear, Submission_time as 'Submission date time', case when ComplianceSchemeId is null then 'DP' else CS_Name end as 'Submitted by',	File_Status as 'Submission status', Regulator_Status as 'Regulator Decision', Actual_Regulator_Status as 'Actual Regulator Decision',	[Who submitted], [CS Nation] , cd_filename, ComplianceSchemeId, cd_organisation_size,cd_submission_period_code --YM001
...
--select * from l_org_sql where [Org ID] = '126500'
POM as
(
		select *
