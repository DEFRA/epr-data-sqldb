CREATE VIEW [dbo].[v_SecurityQuery] AS with cte_Enrolments as (
...
cte_OrganisationsAndRolesAndPersons as (
    select a.*
    ,b.*
    from cte_OrganisationsAndRoles a
    left join cte_PersonOrgConnectionsAndEnrolments b
    on a.Organisations_Id = b.PersonOrganisationConnections_OrganisationId
),
...
cte_OrganisationsAndRolesAndPersonsAndNations as (
    select a.*
    ,b.Name as FromOrganisation_NationName
    ,c.Name as ToOrganisation_NationName
    ,ROW_NUMBER() OVER (ORDER BY Organisations_Id) AS Security_Id

    from cte_OrganisationsAndRolesAndPersons a

    left join rpd.Nations b
    on a.FromOrganisation_NationId = b.Id

    left join rpd.Nations c
    on a.ToOrganisation_NationId = c.Id
),


-- Security Query Starts Here
cte_Security as (
    select Security_Id, FromOrganisation_NationName as SecurityQuery--, FromOrganisation_Name as OrgName
    from cte_OrganisationsAndRolesAndPersonsAndNations
    union all
    select Security_Id, ToOrganisation_NationName as SecurityQuery--, ToOrganisation_Name as OrgName
    from cte_OrganisationsAndRolesAndPersonsAndNations
)--,
-- Security Query Ends Here 

select * from cte_Security;