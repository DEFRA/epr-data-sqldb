CREATE VIEW [dbo].[v_Compliance_Report_update]
...
FROM [v_rpd_Organisations_Active] Org
...
LEFT JOIN pom_sub fpom
    ON fpom.referencenumber=Org.referenceNumber AND fpom.rn_asc = 1 and fpom.Relevant_Year=rel.Relevant_year--TRY_CAST('20' + RIGHT(RTRIM(meta.SubmissionPeriod), 2) AS INT)
LEFT JOIN reg_pom lpom
    ON lpom.referencenumber=Org.referenceNumber AND lpom.rn_desc = 1 and lpom.Relevant_Year=rel.Relevant_year--TRY_CAST('20' + RIGHT(RTRIM(meta.SubmissionPeriod), 2) AS INT)
