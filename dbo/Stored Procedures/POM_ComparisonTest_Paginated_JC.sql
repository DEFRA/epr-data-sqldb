...
BEGIN
    ...
    /*
 IF OBJECT_ID('dbo.POM_COMP_JC', 'U') IS NOT NULL
BEGIN
    -- Drop the table if it exists
    DROP TABLE dbo.POM_COMP_JC;
END
*/
...
select * into JC_TEST from #file_joined
      ...
      SELECT distinct
       ...
       ,fj.packaging_type, fj.packaging_material,file1_Quantity_kg_extrapolated,file2_Quantity_kg_extrapolated
        ,fj.from_nation, fj.relative_move, fj.packaging_activity, fj.packaging_class, fj.packaging_sub_material, fj.to_nation
        ,fj.org_name,  fj.Quantity_kg_extrapolated_diff
 

       ,CASE 
		WHEN fj.packaging_type IN ('Total Non-Household packaging',
		...
		ELSE NULL 
	END Total_Packaging, r.[org_sub_type] org_sub_type2, 
    case when r.registration_type_code = 'GR' then 'Group'
   when r.registration_type_code = 'IN' then 'Single'
   else '' end  [registration_type_code2]
    FROM #file_joined fj
left join t_registration_latest r on r.organisation_id = fj.OrganisationName and r.subsidiary_id = fj.subsidiary_id
--and r.SecurityQuery = @securityquery
