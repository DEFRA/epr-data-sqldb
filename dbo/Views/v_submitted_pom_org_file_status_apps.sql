CREATE VIEW [dbo].[v_submitted_pom_org_file_status_apps] AS With
/****************************************************************************************************************************
	History: 
...
,res as (
		select distinct
				...
				, CASE WHEN cfm.ComplianceSchemeId is not null THEN 'CS' ELSE 'DP' END AS CSorDP
				, p.[FirstName] + ' ' + [LastName] AS Submitted_by
				, p.[Email]
				, p.[Telephone]
				, sr.[Name] AS Account_role
				, org.[ReferenceNumber] AS Organisation_ID
		...
		left join [rpd].[Organisations] org on org.[ExternalId] = cfm.[OrganisationId]	
