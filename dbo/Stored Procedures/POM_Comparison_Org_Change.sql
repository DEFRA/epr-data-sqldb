...
SELECT 
	...
	ISNULL(organisation_size1, '') Current_Size,
	ISNULL(organisation_size2, '') Previous_Size,
	CASE
		WHEN organisation_id2 IS NULL 
			THEN 'Org Added'
		WHEN organisation_id1 IS NULL 
			THEN 'Org Removed'
		WHEN organisation_size1 = 'L' AND ISNULL(organisation_size2, 'S') = 'S'
			THEN 'Size Increased'
		WHEN organisation_size1 = 'S' AND ISNULL(organisation_size2, 'L') = 'L'
			THEN 'Size Decreased'
		WHEN organisation_size2 IS NOT NULL AND organisation_size1 IS NULL
			THEN 'Size Removed'
        ELSE 'No Change'
	END
	Org_Change
