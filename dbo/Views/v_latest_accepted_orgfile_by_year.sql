CREATE VIEW [dbo].[v_latest_accepted_orgfile_by_year] AS 

WITH base_data AS (
    SELECT 
        m.OrganisationId AS meta_OrganisationId,
        m.SubmissionPeriod,

        TRY_CAST('20' + RIGHT(TRIM(m.SubmissionPeriod), 2) AS INT) - 1 AS ReportingYear,
    ...
    LEFT JOIN rpd.Nations n 
        ON n.id = cs.Nationid
    WHERE UPPER(TRIM(ISNULL(st.Regulator_Status, 'PENDING'))) 
          IN ('ACCEPTED','GRANTED')
),
...
cd_org_combined AS (
    SELECT 
        o.ReferenceNumber AS file_submitted_organisation_reference,
