CREATE VIEW [dbo].[v_New_Enrolment_Report_AV] AS WITH
/****************************************************************************************************************************
	History:

	Created: 2025-05-28:	AV001:	Ticket - 555905: Decision_date should not show for 'Nominated','Invited' and 'pending' status.  Added Case statement to display Null for the decision date for the statuses 'Nominated','Invited' and 'pending'.
...
select sec.FromOrganisation_Name
		...
		--AV001 added Case statement for when the status is 'Nominated','Invited' and 'pending' the decision date is Null and if it is any other status it displays the date as normal.
		,Decision_Date = CASE 
			WHEN ve.[Status] = 'Nominated' OR ve.[Status] = 'Invited' OR ve.[Status] = 'Pending' THEN NULL
