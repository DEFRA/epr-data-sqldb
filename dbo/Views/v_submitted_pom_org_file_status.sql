...
--SET A (Decision records - related to submissions)
resubmission_list_A as
(
...
(
	select CONVERT(DATETIME,substring(se.Created,1,23)) as Created_ts , se.SubmissionEventId, se.SubmissionId
	 ...
	 where se.type = 'RegulatorRegistrationDecision'
			...
			--and se.SubmissionId = 'b7efef91-2bda-488b-9ad7-32c310a3087f'
),
...
final_result_set_A as
(
select distinct * from rank_list_A where RN = 1
),


--SET B (Canclled decision record after resubmission)
resubmission_list_B as
(
	select SE.SubmissionId, 1 as IsResubmitted, min(CONVERT(DATETIME,substring(SE.Created,1,23))) as first_resub_ts
	from rpd.submissions S
	inner join rpd.SubmissionEvents SE on S.SubmissionId = SE.SubmissionId 
...
final_result_set_B as
(
select distinct * from rank_list_B where RN = 1
),


--SET C (Non Cancelled decision record after resubmission)
resubmission_list_C as
(
	select SE.SubmissionId, 1 as IsResubmitted, min(CONVERT(DATETIME,substring(SE.Created,1,23))) as first_resub_ts
	from rpd.submissions S
	inner join rpd.SubmissionEvents SE on S.SubmissionId = SE.SubmissionId 
...
final_result_set_C as
(
select distinct * from rank_list_C where RN = 1
),
...
SELECT DISTINCT
    pom_cd.SubmissionId,
    pom_cd.RegistrationSetId,
    pom_cd.OrganisationId,
    pom_cd.FileName,
    pom_cd.FileType,
    pom_cd.OriginalFileName,
    pom_cd.TargetDirectoryName,
    pom_cd.ComplianceSchemeId,
    pom_cd.Decision_Date,
    pom_cd.Regulator_Status_recalculated AS Regulator_Status,
    pom_cd.RegulatorDecision,
    pom_cd.Regulator_User_Name,
    pom_cd.Regulator_Rejection_Comments,
    pom_cd.RejectionComments,
    pom_cd.Type,
    pom_cd.UserId,
    pom_cd.RowNumber,
    pom_cd.Created,
    TRY_CONVERT(DATETIME, SUBSTRING(pom_cd.Created, 1, 23)) AS CreatedDateTime,
    pom_cd.Application_submitted_ts,
    pom_cd.RegistrationType,
    pom_cd.SubmissionPeriod,
    TRY_CONVERT(INT, RIGHT(pom_cd.SubmissionPeriod, 4)) AS SubmissionPeriodYear,
    pom_cd.ApplicationReferenceNo,
    pom_cd.registrationreferencenumber,
    pom_cd.Original_Regulator_Status,
    pom_cd.SubmissionType,
    pom_cd.IsResubmission_identifier,
    pom_cd.Is_resubmitted_POM_identifier,
    pom_cd.cfm_FileId,
    pom_cd.FileId,
    pom_cd.fileid_new,
    pom_cd.submitted_Fileid,
    pom_cd.SubmissionEventId_of_submitted_record,
    pom_cd.app_submitted_Fileid,
    pom_cd.SubmissionEventId_of_application_submitted_record
...
SELECT DISTINCT
    ...
    cfm.ComplianceSchemeId,
    cp.Decision_Date,
    cp.Regulator_Status_recalculated AS Regulator_Status,
    cp.RegulatorDecision,
    cp.Regulator_User_Name,
    cp.Regulator_Rejection_Comments,
    cp.RejectionComments,
    cp.Type,
    cp.UserId,
    cp.RowNumber,
    cp.Created,
    TRY_CONVERT(DATETIME, SUBSTRING(cp.Created, 1, 23)) AS CreatedDateTime,
    cp.Application_submitted_ts,
    cp.RegistrationType,
    cp.SubmissionPeriod,
    TRY_CONVERT(INT, RIGHT(cp.SubmissionPeriod, 4)) AS SubmissionPeriodYear,
    cp.ApplicationReferenceNo,
    cp.registrationreferencenumber,
    cp.Original_Regulator_Status,
    cp.SubmissionType,
    cp.IsResubmission_identifier,
    cp.Is_resubmitted_POM_identifier,
    cp.cfm_FileId,
    cp.FileId,
    cp.fileid_new,
    cp.submitted_Fileid,
    cp.SubmissionEventId_of_submitted_record,
    cp.app_submitted_Fileid,
    cp.SubmissionEventId_of_application_submitted_record
FROM rpd.cosmos_file_metadata cfm
INNER JOIN cd_pom_result_updated cp
    ON cfm.RegistrationSetId = cp.RegistrationSetId
